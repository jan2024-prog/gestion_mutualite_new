...
-- =======================
-- CREDIT avec gestion des devises
-- =======================
CREATE TABLE credit (
    ...
    devise ENUM('Franc','Dollar') DEFAULT 'Franc', -- Nouvelle colonne devise
    date_credit DATE,
    date_echeance DATE,
...
CREATE TABLE mouvement_caisse (
    ...
    origine ENUM('Cotisation','Retrait','Credit','Remboursement','Modification Remboursement', 'Suppression Remboursement','Correction') NOT NULL,
...
BEGIN
    ...
    -- 🔹 Identifier la caisse active (ici, la première caisse)
    SELECT id_caisse INTO id_caisse_actuelle
    FROM caisse
    LIMIT 1;
    ...
    -- 🔹 Enregistrer le mouvement dans mouvement_caisse
    INSERT INTO mouvement_caisse (
        id_caisse,
        id_utilisateur,
        type_mouvement,
    ...
    VALUES (
        id_caisse_actuelle,
        NULL,  -- remplacer par l'id de l'utilisateur connecté depuis PHP si nécessaire
        'Entrée',
        'Remboursement',
    ...
    -- 🔹 Mettre à jour le crédit : reste et statut
    UPDATE credit
...
BEGIN
    ...
    VALUES (
        ...
        CASE 
            WHEN NEW.montant_rembourse - OLD.montant_rembourse >= 0 THEN 'Entrée'
            ELSE 'Sortie'
        END,
        'Modification Remboursement',
        ABS(NEW.montant_rembourse - OLD.montant_rembourse),
        CONCAT('Modification Remboursement Crédit ID ', NEW.id_credit),
        CURDATE(),
        NEW.devise
    );

    -- 🔹 Recalculer le reste du crédit
    SET nouveau_reste = (SELECT montant_credit FROM credit WHERE id_credit = NEW.id_credit)
                        - (SELECT IFNULL(SUM(montant_rembourse),0)
                           FROM remboursement
                           WHERE id_credit = NEW.id_credit);
...
CREATE TABLE IF NOT EXISTS Tcompte_bloque (
    ...
    solde_franc DECIMAL(12,2) DEFAULT 0,
    solde_dollar DECIMAL(12,2) DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS Tdepot_compte_bloque (
    ...
    libele VARCHAR(100),
    devise ENUM('Franc','Dollar') DEFAULT 'Franc',
...
BEGIN
    ...
    DECLARE solde_fr DECIMAL(12,2);
    DECLARE solde_usd DECIMAL(12,2);

    -- Récupérer les soldes actuels
    SELECT solde_franc, solde_dollar
    INTO solde_fr, solde_usd
    FROM Tcompte_bloque
    WHERE id_compte_bloque = NEW.id_compte_bloque;

    IF NEW.devise = 'Franc' THEN
        SET solde_av = solde_fr;
        SET solde_ap = solde_av - NEW.montant;
        UPDATE Tcompte_bloque SET solde_franc = solde_ap WHERE id_compte_bloque = NEW.id_compte_bloque;
    ELSE
        SET solde_av = solde_usd;
        SET solde_ap = solde_av - NEW.montant;
        UPDATE Tcompte_bloque SET solde_dollar = solde_ap WHERE id_compte_bloque = NEW.id_compte_bloque;
    END IF;

    -- Historique
    INSERT INTO historique_compte_bloque (
        id_compte_bloque, type_operation, montant, devise,
        mode_paiement, libelle, solde_avant, solde_apres
    ) VALUES (
        NEW.id_compte_bloque, 'Retrait', NEW.montant, NEW.devise,
        NEW.mode_paiement, NEW.libelle, solde_av, solde_ap
    );

    -- Suppression si solde <= 0
    IF solde_ap <= 0 AND
       ((NEW.devise = 'Franc' AND solde_usd = 0) OR
        (NEW.devise = 'Dollar' AND solde_fr = 0))
    THEN
...
AFTER INSERT ON TmembreBl
FOR EACH ROW
BEGIN
    INSERT INTO Tcompte_bloque (
        id_membre,
...
BEGIN
    IF NEW.devise = 'Franc' THEN
        UPDATE Tcompte_bloque
        SET solde_franc = solde_franc + NEW.montant
        WHERE id_compte_bloque = NEW.id_compte_bloque;
    ELSE
        UPDATE Tcompte_bloque
        SET solde_dollar = solde_dollar + NEW.montant
        WHERE id_compte_bloque = NEW.id_compte_bloque;
    END IF;
END$$
...
FOR EACH ROW
BEGIN
    IF OLD.devise = 'Franc' THEN
        UPDATE Tcompte_bloque
        SET solde_franc = solde_franc - OLD.montant
        WHERE id_compte_bloque = OLD.id_compte_bloque;
    ELSE
        UPDATE Tcompte_bloque
        SET solde_dollar = solde_dollar - OLD.montant
        WHERE id_compte_bloque = OLD.id_compte_bloque;
    END IF;
END$$
...
BEGIN
    IF NEW.devise = 'Franc' THEN
        UPDATE Tcompte_bloque
        SET solde_franc = solde_franc + (NEW.montant - OLD.montant)
        WHERE id_compte_bloque = NEW.id_compte_bloque;
    ELSE
        UPDATE Tcompte_bloque
        SET solde_dollar = solde_dollar + (NEW.montant - OLD.montant)
        WHERE id_compte_bloque = NEW.id_compte_bloque;
    END IF;
END$$
...
BEGIN
    ...
    IF NEW.devise = 'Franc' THEN
        SELECT solde_franc INTO solde_actuel
        FROM Tcompte_bloque
        WHERE id_compte_bloque = NEW.id_compte_bloque
        FOR UPDATE;
    ELSE
        SELECT solde_dollar INTO solde_actuel
        FROM Tcompte_bloque
        WHERE id_compte_bloque = NEW.id_compte_bloque
        FOR UPDATE;
    END IF;
    ...
    IF solde_actuel < NEW.montant THEN
        ...
        SET MESSAGE_TEXT = 'Retrait refusé : solde insuffisant pour cette devise';
    END IF;
END$$
...
BEGIN
    ...
    IF NEW.devise = 'Franc' THEN
        SELECT solde_franc + OLD.montant
        INTO solde_disponible
        FROM Tcompte_bloque
        WHERE id_compte_bloque = NEW.id_compte_bloque
        FOR UPDATE;
    ELSE
        SELECT solde_dollar + OLD.montant
        INTO solde_disponible
        FROM Tcompte_bloque
        WHERE id_compte_bloque = NEW.id_compte_bloque
        FOR UPDATE;
    ...
    IF solde_disponible < NEW.montant THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Modification refusée : solde insuffisant pour cette devise';
    END IF;
END$$

DELIMITER ;

CREATE TABLE caisse_compte_bloque (
    id_caisse_cb INT AUTO_INCREMENT PRIMARY KEY,
    devise ENUM('Franc','Dollar') NOT NULL,
    solde DECIMAL(14,2) DEFAULT 0,
    date_update DATE
);

CREATE TABLE mouvement_caisse_compte_bloque (
    id_mouvement INT AUTO_INCREMENT PRIMARY KEY,
    id_caisse_cb INT,
    type_mouvement ENUM('Entrée','Sortie') NOT NULL,
    origine ENUM('Depot','Retrait','Correction') NOT NULL,
    montant DECIMAL(14,2) NOT NULL,
    devise ENUM('Franc','Dollar') DEFAULT 'Franc',
    libelle VARCHAR(150),
    date_mouvement DATE,

    FOREIGN KEY (id_caisse_cb)
        REFERENCES caisse_compte_bloque(id_caisse_cb)
        ON DELETE CASCADE
);

-- MODIFICATION — TRIGGER DÉPÔT COMPTE BLOQUÉ
-- AFTER INSERT DÉPÔT

DELIMITER $$

CREATE TRIGGER trg_depot_cb_caisse
AFTER INSERT ON Tdepot_compte_bloque
FOR EACH ROW
BEGIN
    DECLARE id_caisse_cb_actuelle INT;

    -- Identifier la caisse par devise
    SELECT id_caisse_cb INTO id_caisse_cb_actuelle
    FROM caisse_compte_bloque
    WHERE devise = NEW.devise
    LIMIT 1;

    -- Augmenter la caisse
    UPDATE caisse_compte_bloque
    SET solde = solde + NEW.montant,
        date_update = CURDATE()
    WHERE id_caisse_cb = id_caisse_cb_actuelle;

    -- Mouvement caisse
    INSERT INTO mouvement_caisse_compte_bloque (
        id_caisse_cb,
        type_mouvement,
        origine,
        montant,
...
-- MODIFICATION — TRIGGER RETRAIT COMPTE BLOQUÉ
-- AFTER INSERT RETRAIT

DELIMITER $$

CREATE TRIGGER trg_retrait_cb_caisse
AFTER INSERT ON Tretrait_compte_bloque
FOR EACH ROW
BEGIN
    DECLARE id_caisse_cb_actuelle INT;
...
-- APRÈS MODIFICATION D’UN RETRAIT (AJUSTEMENT CAISSE)

DELIMITER $$

CREATE TRIGGER trg_retrait_cb_after_update
AFTER UPDATE ON Tretrait_compte_bloque
FOR EACH ROW
BEGIN
    DECLARE id_caisse_old INT;
    DECLARE id_caisse_new INT;
    DECLARE diff DECIMAL(12,2);

    -- Caisse de l'ancienne devise
    SELECT id_caisse_cb INTO id_caisse_old
    FROM caisse_compte_bloque
    WHERE devise = OLD.devise
    LIMIT 1;

    -- Caisse de la nouvelle devise
    SELECT id_caisse_cb INTO id_caisse_new
    FROM caisse_compte_bloque
    WHERE devise = NEW.devise
    LIMIT 1;

    -- CAS 1 : même devise → ajustement
    IF OLD.devise = NEW.devise THEN
        SET diff = NEW.montant - OLD.montant;

        UPDATE caisse_compte_bloque
        SET solde = solde - diff,
            date_update = CURDATE()
        WHERE id_caisse_cb = id_caisse_new;

        INSERT INTO mouvement_caisse_compte_bloque (
            id_caisse_cb, type_mouvement, origine,
            montant, devise, libelle, date_mouvement
        ) VALUES (
            id_caisse_new,
            IF(diff > 0, 'Sortie', 'Entrée'),
            'Correction',
            ABS(diff),
            NEW.devise,
            CONCAT('Modification retrait ID ', NEW.id_retrait),
            CURDATE()
        );

    -- CAS 2 : devise changée
    ELSE
        -- Restituer à l’ancienne caisse
        UPDATE caisse_compte_bloque
        SET solde = solde + OLD.montant,
            date_update = CURDATE()
        WHERE id_caisse_cb = id_caisse_old;
        ...
        INSERT INTO mouvement_caisse_compte_bloque (
            ...
            OLD.montant,
            OLD.devise,
            CONCAT('Annulation retrait ID ', OLD.id_retrait),
            CURDATE()
        );

        -- Appliquer à la nouvelle caisse
        UPDATE caisse_compte_bloque
        SET solde = solde - NEW.montant,
            date_update = CURDATE()
        WHERE id_caisse_cb = id_caisse_new;
        ...
        INSERT INTO mouvement_caisse_compte_bloque (
            ...
            NEW.montant,
            NEW.devise,
            CONCAT('Nouveau retrait ID ', NEW.id_retrait),
            CURDATE()
        );
    END IF;
END$$
...
-- APRÈS SUPPRESSION D’UN RETRAIT (RESTITUTION CAISSE)

DELIMITER $$

CREATE TRIGGER trg_retrait_cb_after_delete
AFTER DELETE ON Tretrait_compte_bloque
FOR EACH ROW
BEGIN
    DECLARE id_caisse_cb_actuelle INT;
    ...
    -- Restituer le montant à la caisse
    UPDATE caisse_compte_bloque
    SET solde = solde + OLD.montant,
        date_update = CURDATE()
    WHERE id_caisse_cb = id_caisse_cb_actuelle;
    ...
    INSERT INTO mouvement_caisse_compte_bloque (
        ...
        OLD.montant,
        OLD.devise,
        CONCAT('Suppression retrait ID ', OLD.id_retrait),
        CURDATE()
    );
END$$
...
--  AVANT UTILISATION 
-- 2. Initialiser les caisses
INSERT INTO caisse_compte_bloque (devise, solde, date_update)
VALUES ('Franc',0,CURDATE()),('Dollar',0,CURDATE());
